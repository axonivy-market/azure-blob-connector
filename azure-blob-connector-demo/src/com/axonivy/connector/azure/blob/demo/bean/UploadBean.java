...
public class UploadBean extends AbstractDemoBean {
	...
	public void init() {
		...
		Credential tokenCredential = new ClientSecretCredential(tenantId, clientId, clientSecret);
		storageService = new AzureBlobStorageService(tokenCredential, endPoint, containerName);
	...
	public void uploadFromPath() {
		...
		isOverwriteFilePath = isFileAlreadyExist(uploadToFolderByLocalPath, fileNamePath, isOverwriteFilePath);
		if (isOverwriteFilePath) {
			return;
		}
